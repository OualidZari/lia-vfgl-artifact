...
function run_experiment() {
    ...
    python main.py --dataset ${dataset} --device cuda --gpu 3 --seed ${seed} --lr 0.01 --epochs 300 --val_ratio 0.02 \
        ...
        --use_wandb \
        --experiment_comment "Artifact Table 4" \
        --attack_epochs [-1] \
...
architectures=('gcn')
datasets=('Cora' 'Citeseer' 'amazon_computer' 'amazon_photo' 'Twitch-FR' 'Twitch-DE' 'Twitch-EN')
...
#datasets=('Cora')
parallel -j 5 run_experiment ::: "${seeds[@]}" ::: "${fraction_data_gcn[@]}" ::: "${datasets[@]}" ::: "${architectures[@]}" ::: "${train_ratios[@]}" ::: "${similarity_metric[@]}"
